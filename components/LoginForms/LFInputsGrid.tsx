  ...
  return <View style={{
    ...
    <View style={{
      ...
      <View style={{
        ...
        <LFButton type="secondary" icon="add" label={labelAdd} size="small" onPress={()=>{
          AddNewRow()
        }} />
      </View>
    </View>
    ...
    <View style={[
        ...
        StyleZTable.row,
      {
      ...
      ]}>
        {
          inputs.map((inputObj, key_input)=>{
            return <View key={`grid-row-${key_input}`} style={[
              StyleZTable.cell,
      ...
      </View>
      {
        rows_data.map((obj, key)=>{
          return <View key={`grid-row-${key}`} style={[
              StyleZTable.row
            ]}>
            {
              inputs.map((inputObj:ILFInputGridInput, key_input)=>{
                return <View key={`input-holder-${key}-${key_input}`} style={[
                  StyleZTable.cell,
                  (key_input===0?StyleZTable.cellFirst: null),
                  {
                    width: `${_CellWidth()}%`
            ...
            <View style={[
              ...
              <LFButton size="small" type="secondary" icon="close" onPress={()=>{
                DeleteRow(key)
              }} />
            </View>
          </View>
