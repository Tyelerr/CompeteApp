...
export default function ScreenFAQsContact(){
  ...
  const __SendTheMessage = async ()=>{
    ...
    try {
      ...
      if (supported) {
        ...
        Alert.alert('Error', 'No email app found or unable to open email client. Please ensure you have an email app installed and configured.');
      }
    } catch (error) {
      console.error('An error occurred while trying to open email client:', error);
      Alert.alert('Error', 'Failed to open email client due to an unexpected issue.');
    }
  ...
  return <UIPanel>
    ...
    ]}>Can't find what you're looking for? Send us a message and we'll help you out.</Text>
    <Text style={[
      StyleZ.h4,
      {
    ...
    <View style={[
    ...
    ]}>
      {
        ...
        ]}>Username: {user?.email.split('@')[0]}</Text>
        :
        null
      }
      <Text style={[
        StyleZ.p
      ]}>We'll use this information to respond to your message.</Text>
    </View>
    ...
    <LFInput 
...

      {
        ...
        ?
        <LFInput label="Tournament ID (Optional)" value={tournament_id} placeholder="Enter tournament ID" onChangeText={(text)=>{
      ...
      <LFInput typeInput="textarea" label="Message" placeholder="Please describe your question or issue in detail..." onChangeText={(text)=>{
        set_message( text );
      }} />
